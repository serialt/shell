...
download_redis(){
	network-ping
	if [ ! -f /usr/local/src/redis50.tar.gz ];
        then
...
start_redis(){
	for m in `ls /usr/local/redis/muti-redis/`
	do
...
stop_redis(){
        ...
        for m in `ss -anpl | awk '/redis/{print $5}' | awk -F: '{print $2}'`
        do
...
stop_redis_force(){
	       for m in `ps -ef | awk '/redis-server/{print $2}'`
        do
                kill -9 $m
